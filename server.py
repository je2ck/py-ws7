...
class IndexHandler(tornado.web.RequestHandler):
    ...
    def get(self):
        self.render("index.html", wavelengths=wlmeter.wavelengths, **get_config())
    ...
    def __call__(self, parser, namespace, values, option_string=None):
        ...
        if not os.path.isfile(config_file):
            raise argparse.ArgumentTypeError(
                "config:{0} is not a valid file".format(config_file)
            )
        if os.access(config_file, os.R_OK):
            setattr(namespace, self.dest, config_file)
        else:
            raise argparse.ArgumentTypeError(
                "config:{0} is not a readable file".format(config_file)
            )
...
def get_config():
    ...
    parser = argparse.ArgumentParser(
        description="Starts a webserver with wavemeter interface."
    )
    parser.add_argument(
        "--debug",
        dest="debug",
    ...
    parser.add_argument("port", type=int, nargs="?", help="server port, default: 8000")
    ...
    config = {
        ...
        "channels": [
            {"i": i, "label": "Channel %d" % (i + 1)} for i in range(8)
        ],  # channels to display
    }
...
if __name__ == "__main__":
    ...
    if "ssl" in config:
        ...
        server = tornado.httpserver.HTTPServer(
            app, xheaders=True, ssl_options=config["ssl"]
        )
    else:
        # http and ws server
